...
test("shows an error when email is already exists", async () => {
  ...
  server.use(
    http.post("/api/auth/signup", () => {
      return HttpResponse.json({ message: "EMAIL_ALREADY_EXISTS" }, { status: 409 });
    })
  );
...
test("shows an error when signin credentials are invalid", async () => {
  ...
  expect(await screen.findByText("Invalid username or email")).toBeInTheDocument();
});
...
test("shows an error after too many signin attempts", async () => {
  ...
  server.use(
    http.post("/api/auth/signin", () => {
      return HttpResponse.json({ message: "SIGNIN_LIMIT_REACHED" }, { status: 429 });
    })
  );
