...
export default function Header() {
  ...
  const userMenuRef = useRef<HTMLDivElement>(null); // captures the HTML element
  const mobileMenuRef = useRef<HTMLDivElement>(null);
  const [isUserMenuOpen, setIsUserMenuOpen] = useState(false);
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);

  const emailLocal = user?.email.slice(0, user.email.indexOf("@")); // example; would usually be a username
  ...
  useEffect(() => {
    ...
    const handleMouseDown = (e: MouseEvent) => {
      ...
      // if mousedown is outside of menu div
    ...
    // cleanup useEffect function (is stored when running, then executed first on next run, then repeat)
    return () => {
  ...
  const handleSignout = async () => {
    try {
      await signout();
      await refreshUser(); // sets user to null
    } catch (err) {
  ...
  return (
    <header className="sticky top-0 z-1 flex h-18 items-center justify-between bg-white px-6 dark:bg-black dark:text-white">
      ...
      <div className="flex items-center gap-10">
        <Link to="/" className="flex items-center gap-2">
          ...
          <span className="font-lexend hidden text-xl md:block">{tCommon("app")}</span>
        </Link>
      ...
      {user && (
        <div ref={userMenuRef}>
          ...
          <div
            ...
            // invisible : disables mouse/keyboard interaction on top of hiding it while still being rendered
          >
